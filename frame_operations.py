...
from frame_elements import eval_frame_element, singsupp, Bspline, get_ombx
    ...
    for r in range(len(indexSet)):
        ...
        for n in range(r, len(indexSet)):
            ...
            if boundleft < boundright:
                ...
                # split the integrand
                def integrand1(t):
                    return np.real(np.exp(2 * np.pi * 1j * t * (omega1 - omega2)) * Bspline(m, (t-x1) / beta1) * Bspline(m, (t-x2) / beta2))

                def integrand2(t):
                    return np.imag(np.exp(2 * np.pi * 1j * t * (omega1 - omega2)) * Bspline(m, (t-x1) / beta1) * Bspline(m, (t-x2) / beta2))

                # integrate and sum over all intervals
                for counter in range(len(ssupint) - 1):
                    val = quad(integrand1, ssupint[counter], ssupint[counter + 1])[0] + 1j * quad(integrand2, ssupint[counter], ssupint[counter + 1])[0]
